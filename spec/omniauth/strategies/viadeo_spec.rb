...
describe OmniAuth::Strategies::Viadeo do
  subject { OmniAuth::Strategies::Viadeo.new(nil) }

  it 'should add a camelization for itself' do
    expect(OmniAuth::Utils.camelize('viadeo')).to eq('Viadeo')
  end

  describe '#client' do
    it 'has correct Viadeo site' do
      expect(subject.client.site).to eq('https://api.viadeo.com')
    end

    it 'has correct authorize url' do
      expect(subject.client.options[:authorize_url]).to eq('https://www.viadeo.com/uas/oauth2/authorization?response_type=code')
    end

    it 'has correct token url' do
      expect(subject.client.options[:token_url]).to eq('https://www.viadeo.com/uas/oauth2/accessToken')
    end
  end

  describe '#callback_path' do
    it 'has the correct callback path' do
      expect(subject.callback_path).to eq('/auth/viadeo/callback')
    end
  end
